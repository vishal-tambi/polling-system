...
1. Push the `frontend/` folder to GitHub (or link the monorepo)
...
> The `frontend/vercel.json` includes a redirect rule so page refreshes on any route resolve correctly (SPA routing fix):
> ```json
...
1. Create a new **Railway** project, link your GitHub repo
2. Set root directory to `backend`
3. Add environment variables: `MONGODB_URI`, `FRONTEND_URL`, `PORT`
4. Railway will auto-detect and run `npm start` (make sure `npm run build` runs first)

---

### Uptime Monitoring

The `/health` endpoint is designed for **UptimeRobot** (or similar):

```
GET https://your-backend.railway.app/health
→ 200 OK  { "status": "ok", "timestamp": "2025-01-01T00:00:00.000Z" }
```
...
│  │  (Vite/TSX)  │                    │  (Node.js)       │  │
│  │              │ ◄──────────────── │                  │  │
│  │  PollContext │   WebSocket        │  Socket.io       │  │
│  │  (Socket.io  │ ◄────────────────► │  (Real-time)     │  │
│  │   client)    │                    │                  │  │
│  └──────────────┘                    └────────┬─────────┘  │
│                                               │             │
...
**Data flow for a live vote:**
1. Student clicks an option → `poll:vote` emitted via Socket.io
2. Server calls `PollService.castVote()` → saves `Vote` doc, atomically `$inc` vote count on `Poll`
3. Server emits `poll:vote_updated` to **all** connected clients
4. Every browser's `PollContext` listener updates local state → React re-renders bar charts
...
<div align="center">
  Made with ❤️ for classrooms everywhere · <strong>Intervue Poll</strong>
</div>
